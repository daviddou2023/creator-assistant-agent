...
└── video_review_agent/             # agent 核心代码
    ...
    ├── analytics.py                # 规则分析层
    ├── reporting.py                # 报告渲染
    ├── dashboard.py                # Dashboard 图表数据构造
    ├── service.py                  # Flask REST API 与 SSE 服务
...
5. `data_analyst`：把数字指标转成 Dashboard 图表数据。
6. `infer_content`：归纳观众更感兴趣的选题和呈现方式。
7. `recommend`：生成下一步创作建议。
8. `plan_review`：生成可供前端展示和用户确认的 Plan，必要时中断等待 resume。
9. `render_report`：输出 Markdown 报告，可选择用 LLM 润色。
10. `store_memory`：复盘结束后提取核心经验，向量化写入本地 Qdrant 记忆库。
...
- Flask 服务位于 `server.py` / `video_review_agent/service.py`，Dashboard 页面位于 `templates/` 和 `static/`。
- 默认不依赖 LLM 也能输出稳定报告。
